...
\paragraph{Fonctionnement du plannificateur Elastic Band}



2. Le concept des Bulles (Bubbles)
...
Définition géométrique (cas holonome 2D) : Pour une configuration $b$, si $p(b)$ représente la distance minimale absolue entre le robot et l'obstacle le plus proche, la bulle $B(b)$ est définie par le disque :$$B(b) = \{q : \Vert{}b - q\Vert{} < p(b)\}$$
Tout point situé à l'intérieur de cette bulle garantit l'absence de collision avec l'environnement.  

Condition de continuité de la bande : La bande élastique est constituée d'une chaîne discrète de postures (via-points) associées à leurs bulles respectives. Pour garantir l'existence d'une trajectoire continue sans collision, deux bulles consécutives $B(b_i)$ et $B(b_{i+1})$ doivent obligatoirement se chevaucher ($\text{Overlap} \neq \emptyset$).  

Adaptabilité spatiale : La densité de discrétisation varie automatiquement : en zone dégagée, les bulles sont grandes et espacées ; à proximité des obstacles, les bulles rétrécissent, augmentant la résolution locale du chemin.  

Extension aux robots orientables (3 DDL : $x, y, \theta$) : La métrique de distance intègre la rotation via le rayon maximal $r_{\max}$ de la géométrie du robot par rapport à son centre :
...
3. Modélisation de la déformation par forces artificielles
...
\textbf{Force interne de contraction ($f_c$)} : Elle simule la tension mécanique d'un élastique pour raccourcir le chemin et supprimer le mou (slack) provoqué par les contours inutiles :
...
\textbf{Force externe de répulsion ($f_r$)} : Elle repousse la bande hors des zones à haut risque pour maximiser la marge de sécurité autour des obstacles :
...
\textbf{Force modifiée et stabilité ($f^*$)} : Afin d'éviter le glissement longitudinal indésirable des bulles le long de la bande (ce qui provoquerait des cycles infinis de création/suppression de points), la force totale subie par une bulle est projetée sur l'axe orthogonal à la trajectoire :
