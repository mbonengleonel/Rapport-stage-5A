...
\begin{itemize}
    \item \textbf{Une dérive spatiale critique} : Fortement dépendante de la structure géométrique de l'environnement, la dérive intrinsèque du SLAM kitware engendrait des discontinuités de pose (sauts de localisation) trop importantes lors de la réacquisition du signal GNSS. Pour prévenir tout comportement instable, le module interdisait le basculement si l'écart mesuré dépassait un seuil critique, contraignant le robot à poursuivre sa navigation en mode SLAM dégradé jusqu'à la fin de la mission.
...
\subparagraph{a. Fonctionnement du Kitware SLAM}

Le \textit{Kitware SLAM} repose sur l'algorithme LOAM (\textit{LiDAR Odometry And Mapping in Real-time}) \cite{loam2014}, qui se décompose en trois étapes principales :\\
\begin{enumerate}
    \item Le prétraitement et l'extraction des caractéristiques géométriques du nuage de points LiDAR ;
    \item L'estimation de l'odométrie LiDAR ;
    \item La construction de la carte et l'optimisation spatiale.
\end{enumerate}

\subsubparagraph{Prétraitement et extraction des caractéristiques géométriques}
...
Deux types de caractéristiques sont principalement extraits :
\begin{itemize}
    \item \textbf{Les points d'arêtes (\textit{edge features}) :} ils correspondent aux zones présentant une forte variation de courbure. Ils permettent de contraindre le déplacement autour des structures linéaires (angles de bâtiments, contours d'obstacles).
    \item \textbf{Les points plans (\textit{surface features}) :} ils identifient les zones de faible courbure, représentatives de surfaces régulières (sol, murs, façades).
\end{itemize}
...
\subsubparagraph{Estimation de l'odométrie LiDAR}
La deuxième étape vise à évaluer le déplacement relatif du robot entre deux scans consécutifs. Pour cela, LOAM réalise un recalage \textit{scan-to-scan} entre les caractéristiques extraites de l'acquisition courante et celles de la précédente.\\

L'estimation de la transformation spatiale est obtenue par la minimisation de deux métriques d'erreur :\\
\begin{itemize}
...
\subsubparagraph{Construction de la carte et optimisation}
La troisième étape reconstruit progressivement la carte globale de l'environnement tout en corrigeant la dérive accumulée par l'odométrie.

À l'inverse de l'étape précédente, le recalage s'effectue cette fois entre le scan courant et la carte locale (\textit{scan-to-map}). Les caractéristiques du nouveau nuage de points sont alignées avec la carte préexistante afin d'affiner l'estimation de pose, garantissant ainsi la cohérence spatiale à long terme.\\


Bien que la documentation détaillée du \textit{Kitware SLAM} soit restreinte, cet algorithme intègre un système d'optimisation par graphe de poses \cite{kitware_slam}, capable de réajuster l'ensemble de la carte lors d'une fermeture de boucle (\textit{loop closure}). Il propose également une extraction enrichie de caractéristiques, incluant la détection de ruptures d'intensité lumineuse ou d'amas de points (\textit{blobs}).
De plus, le module offre la possibilité d'injecter des données exogènes issues d'autres capteurs (IMU, GNSS, INS, odométrie des roues, caméras RGB pour balises visuelles). Toutefois, les essais expérimentaux menés n'ont pas révélé de gain de performance significatif lors de l'intégration directe de l'IMU au sein du SLAM. Cela a justifié le choix de conserver une version exclusivement LiDAR pour le SLAM et de coupler l'IMU de manière externe afin d'élever la fréquence de la localisation. Enfin, certaines fonctionnalités avancées ont dû être restreintes afin d'assurer le respect des contraintes temps réel de la pile logicielle (\textit{stack}).



\subparagraph{b. Fonctionnement du DLIO SLAM}

L'algorithme DLIO \cite{dlio2023} est une méthode d'odométrie LiDAR-inertielle sobre en calcul et adaptée aux mouvements brusques. Contrairement aux approches fondées sur l'extraction de primitives géométriques (LOAM, Kitware SLAM), DLIO adopte une approche directe opérant directement sur le nuage de points dense.

Son traitement repose sur quatre étapes clés : \\
\begin{enumerate}
    \item \textbf{Prétraitement dense :} Alignement spatio-temporel des données LiDAR ($10\text{--}20\text{ Hz}$) et IMU ($100\text{--}500\text{ Hz}$) sans sous-échantillonnage agressif ni extraction de descripteurs.
    \item \textbf{Correction de mouvement (\textit{deskewing}) :} Compensation analytique des distorsions de balayage par une approche \textit{coarse-to-fine} (intégration IMU et interpolation en forme fermée), générant l'estimation \textit{a priori} $\hat{\mathbf{T}}_{\text{prior}}^{\mathcal{W}}$.
    \item \textbf{Recalage géométrique (GICP) :} Alignement direct \textit{scan-to-map} du nuage corrigé sur une sous-carte locale d'images-clés (\textit{keyframes}) par minimisation de la distance de Mahalanobis \cite{gicp2009}.
    \item \textbf{Fusion et mise à jour d'état :} Observateur géométrique non linéaire à convergence garantie, fusionnant le recalage GICP et les mesures IMU pour estimer l'état complet :
    \begin{equation}
    ...
    \end{equation}
    comprenant la position, l'orientation, la vitesse et les biais de l'IMU.
\end{enumerate}
...
\begin{figure}[h]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Corps_rapport/dlio_APE_plot_raw.png}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Corps_rapport/kitware_APE_plot_raw.png}
        \caption{Erreur absolue de position en Kitware}
        \label{fig:image2}
    \end{subfigure}    
    \caption{Comparaison des erreurs absolues de position en DLIO vs en Kitware}
    \label{fig28}
\end{figure}
$\ $\
Au regard des figures (\hyperref[fig27]{Figure 27} et \hyperref[fig28]{Figure 28}) ci-dessus, on constate que le SLAM DLIO, grâce à son couplage inertiel-LiDAR, garantit une très bonne localisation, même en milieu dégagé. Ce n'est pas le cas du SLAM Kitware, qui montre des limites évidentes dans ces conditions. De plus, le SLAM DLIO présente une erreur de position maximale d'environ 1,5 mètre, tandis que celle du SLAM Kitware atteint environ 80 mètres sur un parcours de 1 km. Ces résultats confortent notre choix de changer de SLAM dans le cadre du défi 3.\\
...
\indent Cette checklist présentée en annexe \ref{annex4:test_checklist} n'étant pas exhaustive, elle a été complétée par des points de contrôle pertinents supplémentaires en fonction des besoins et des observations faites lors des tests.\\
...
\indent Comme résultat, l'outil génère plusieurs représentations graphiques, dont une frise chronologique (timeline) des diagnostics (\hyperref[fig32]{Figure 32}), indexée par identifiant d'alerte. Cette visualisation temporelle permet d'identifier immédiatement les éléments où des anomalies sont survenues, puis de fournir un résumé sur ces diagnostics anormaux détectés. Au sein de la pile logicielle (stack) MOBITER, les diagnostics sont hiérarchisés et catégorisés en quatre grands groupes fonctionnels (niveau) :\\
...
\indent Pour le cas de test illustré en \hyperref[fig33]{Figure 33}, on observe que l'opérateur a repris la main sur le robot en mode téléopération à 4 reprises durant le suivi de waypoints. Cela pourrait traduire une difficulté pour le robot à rallier certains points de passage en toute autonomie.\\
...
\indent Concernant le cas de test illustré par la \hyperref[fig34]{Figure 34}, on observe que le robot a changé de mode de localisation à 4 reprises durant le suivi de waypoints. Cela pourrait traduire plusieurs passages sous couvert végétal le long du trajet.
...
\begin{itemize}
\renewcommand{\labelitemi}{$\bullet$}
    \item \textbf{Des analyses temporelles} : l'évolution des positions linéaires et angulaires du robot au fil du temps.(\hyperref[fig36]{Figure 36})
    \item \textbf{Des comparaisons spatiales} : la superposition des trajectoires issues du SLAM et de la centrale inertielle SBG.(\hyperref[fig37]{Figure 37})
...
\renewcommand{\labelitemi}{$\bullet$}
    ...
    \item \textbf{Modes de localisation} : Visualisation temporelle et géographique du mode de localisation actif (SLAM ou GNSS RTK) sur les différentes portions de la trajectoire.(\hyperref[fig37]{Figure 37})
    ...
    \item \textbf{Analyse des écarts de vitesse} : Évaluation des erreurs de vitesse instantanées par rapport aux vitesses de consigne requises pour le ralliement des waypoints.(\hyperref[fig37]{Figure 37})
...
Comme résultat, l'outil génère plusieurs graphiques dont les figures (\hyperref[fig36]{Figure 36} et \hyperref[fig37]{Figure 37}) suivantes :\\
...
\indent La figure \hyperref[fig36]{Figure 36} illustre la très bonne estimation de la pose du robot par le SLAM DLIO par rapport à la vérité terrain fournie par la centrale SBG. On observe que les positions linéaires et les orientations angulaires estimées par le SLAM sont très proches de celles mesurées par la centrale inertielle, ce qui confirme la précision et la fiabilité du SLAM DLIO pour la navigation.
...
\indent La figure (\hyperref[fig39]{Figure 39}) suivante illustre un exemple de vidéo composite générée par l'outil, montrant l'évolution des cartes et du retour vidéo sur l'IHM. Cette vidéo composite offre ainsi une vue d'ensemble du comportement du système durant les essais et permet d'observer, dans l'ordre de lecture (de gauche à droite et de haut en bas) :\\
