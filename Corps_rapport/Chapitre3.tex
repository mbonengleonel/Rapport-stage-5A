...
\begin{itemize}
    ...
    \item \textbf{Définition géométrique (cas holonome 2D)} : Pour une configuration $b$, si $p(b)$ représente la distance minimale absolue entre le robot et l'obstacle le plus proche, la bulle $B(b)$ est définie par le disque :$$B(b) = \{q : \Vert{}b - q\Vert{} < p(b)\}$$
...
\paragraph{Résultat de l'implémentateur de  l'Elastic Band sur le mode de suivi de lisière}
