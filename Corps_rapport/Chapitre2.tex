...
\indent Une optimisation de cette brique a été réalisée dans le cadre du défi 3. Elle repose sur la mise en place d'une structure d'accélération qui encode l'orientation du robot à chaque instant des arcs de trajectoire, puis la reprojette dans la LUT (\textit{Look-Up Table}) d'origine. Cette approche évite le stockage systématique des empreintes du robot pour l'ensemble des orientations le long des différents arcs : ce sont désormais les points LiDAR qui sont transformés dans le repère de la LUT d'origine, et non plus le modèle du robot qui est décliné selon toutes ses orientations suivant l'arc. Grâce à ce changement de repère, l'empreinte mémoire des LUTs embarquées sur le robot a été réduite de près de 20~Go à moins de 1~Go.
