...
\indent Un schéma synoptique de l'architecture du système est donné ci-dessous (\hyperref[fig13]{Figure 13}):\\
\begin{itemize}
...
\begin{center}
    \includegraphics[width=1.05 \linewidth]{Corps_rapport/architecture_logicielle.png}
...
\indent On vient ensuite seuiller notre carte de gradients afin d'obtenir une information correspondant aux différentes hauteurs d’obstacles à différencier pour le défi. La superposition de ces cartes permet d'obtenir une représentation des obstacles autour du robot comme illustré (\hyperref[fig15]{Figure 15}) :\\
\begin{center}
    \includegraphics[width=0.5\linewidth]{Corps_rapport/Carte_obstacles.png}
...
La brique de segmentation d'image* permet d'extraire des informations sémantiques à partir d'une image caméra. Pour ce faire, elle utilise une combinaison de deux réseaux de neurones : un détecteur open-set, YOLO World \cite{yoloworld2024}, et un réseau de segmentation*, Efficient SAM \cite{efficientsam2024}.\\\\
...
\indent L'objectif à terme est de fusionner cette information sémantique dans la brique du Modèle Numérique d'Environnement (MNE) pour ajouter une notion de traversabilité aux obstacles de la carte.
...
\indent La \hyperref[fig17]{Figure 17} est une représentation étape par étape de l'exploration d'une carte par l'algorithme de Dijkstra. Seuls les mouvements (haut, bas, gauche, droite) sont admissibles. La case bleue est le point de départ de l'algorithme, la case verte l'objectif à atteindre et le cercle jaune le noeud en cours d'exploration :
\newpage
\begin{figure}[h]
    \centering
...
\indent L’algorithme de détection commence par la détection du départ de la lisière à gauche et à droite du robot. Pour cela, on translate un cercle jusqu’à ce que celui-ci intersecte un point. Ce point correspond au premier point de la lisière (\hyperref[fig19]{Figure 19}) :\\
\begin{center}
    \includegraphics[width=0.5\linewidth]{Corps_rapport/lisiere1.png}
    \captionof{figure}{Détection du premier point}
    \label{fig19}
\end{center}
$\ $\
\indent Ensuite, on fait pivoter le cercle pour capturer l'ensemble des points de contour de la lisière (\hyperref[fig20]{Figure 20}) :\\
 \begin{center}
    \includegraphics[width=0.5\linewidth]{Corps_rapport/lisiere2.png}
    \captionof{figure}{Détection du contour de la lisière}
    \label{fig20}
\end{center}
$\ $\
\indent Une fois les points de contour identifiés, des points intermédiaires sont ajoutés pour créer un nuage de points de chaque côté, formant une structure serpentine. Ces "serpents" représentent les bords estimés des lisières scannées (\hyperref[fig21]{Figure 21}) :\\
 \begin{center}
    \includegraphics[width=0.5\linewidth]{Corps_rapport/lisiere3.png}
...
\indent Une fois les lisières détectées, elles sont décalées suivant leur direction normale moyenne et envoyées à l’algorithme de suivi comme trajectoires à suivre.\\\\
\indent Le module de suivi de trajectoire met en jeu un algorithme de suivi de chemin permettant une régulation latérale et longitudinale.\\\\
...
\indent Des primitives de déplacement de type “arc” en vue de dessus sont évaluées sur un horizon temporel de $2\ s$.  Chaque arc est paramétré par une paire de vitesses $(v_{2d},\dot{\alpha}_z)$ constantes sur la fenêtre temporelle.\\

\begin{center}
    \includegraphics[width=0.4\linewidth]{Corps_rapport/DWA_illustration.png}
...
Cette brique est responsable de la communication avec le Barakuda via les trames UDP. D’une part, les informations remontées par le robot sont distribuées au reste du système sous forme de diagnostics. D’autre part, les consignes corrigées par le correcteur cinématique sont envoyées au robot, selon les formules habituelles \eqref{eq4} :\\
...
Cette brique récupère l'ensemble des commandes fournies par les fonctions de pilotage et permet à l'opérateur de choisir laquelle est envoyée au robot via une liste déroulante dans l'IHM.\\\\
\indent Cette brique est également responsable d'une partie de la sécurité du robot puisqu'elle peut limiter la vitesse de ce dernier tout en gardant la même trajectoire lorsque la commande reçue provoque un dépassement du seuil de l'accélération centrifuge autorisé ou encore provoquer l'arrêt et le serrage des freins lors d'un dysfonctionnement grave retourné par une brique. La commande vérifiée ou limitée est ensuite envoyée au correcteur de taux de lacet.
...
\indent La projection dans l’image des informations définies dans le repère monde a été réalisée à l'aide de la calibration intrinsèque de la caméra ainsi que de la calibration extrinsèque caméra/LIDAR* réalisée auparavant et du système de transformations tf de ROS* permettant de définir une matrice de transformation homogène 4x4 pour projeter les éléments du repère monde vers le repère image. Cette transformation est utilisée pour la représentation des obstacles, des waypoints, des flaques d'eau, des végétations traversables et des chemins.\\\\
\indent On obtient alors le retour annoté \hyperref[fig23]{Figure 23} :
\begin{center}
    \includegraphics[width=1\linewidth]{Corps_rapport/viz.png}
    \captionof{figure}{Retour visuel annoté en simulation}
    \label{fig23}
\end{center}
\subsubsection{IHM}
Enfin, la brique de l'IHM fournit une interface graphique (\hyperref[fig24]{Figure 24}) sur le PC débarqué à l'aide du framework ROS* RQT, permettant au téléopérateur d'accéder au retour visuel annoté, de donner des ordres de mission au robot tout en pouvant modifier les paramètres à sa guise, de sauvegarder des trajectoires pour les réutiliser lors des modes rejeu et retour sur traces, de pouvoir entrer des coordonnées de points à atteindre et accéder à un ensemble de données et de diagnostics pour du debug.\\
\begin{center}
    \includegraphics[width=1\linewidth]{Corps_rapport/rqt.png}
    \captionof{figure}{Interface RQT}
    \label{fig24}
\end{center}

\indent En parallèle de l'interface RQT, une visualisation RVIZ (\hyperref[fig25]{Figure 25}) peut être lancée, permettant de visualiser les différentes cartes générées par la brique du MNE, les chemins planifiés et permettant de renseigner un point à atteindre directement sur la carte.\\
\begin{center}
    \includegraphics[width=1\linewidth]{Corps_rapport/rviz.png}
