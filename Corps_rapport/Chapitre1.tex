...
\indent À l'issue de l'appel à projets, sept consortiums ont été retenus comme ci-après (\hyperref[fig3]{Figure 3}) :
...
%     \item \textbf{ASTRA}, d'\textbf{ESIGELEC IRSEEM} en partenariat avec la \textbf{Société d'innovations technologiques industrielles avancées (SITIA)}.\\
%     \item \textbf{MEEDIAN}, de \textbf{Safran Electronics \& Defense} en partenariat avec l'\textbf{Ecole nationale supérieure des mines de Paris CAOR}.\\
%     \item \textbf{NAVIR}, de \textbf{HawAI.tech} en partenariat avec \textbf{INRIA Grenoble - Rhône-Alpes}.\\
%     \item \textbf{PANAME}, de l'\textbf{ONERA} en partenariat avec \textbf{NGX Robotics}.\\
%     \item \textbf{REFLEX}, de \textbf{MAGELLIUM} en partenariat avec le \textbf{CEA-List le laboratoire du CEA}.\\
%     \item \textbf{TAURUS}, du \textbf{Computer Science and Systems Engineering Laboratory (U2IS) de l'ENSTA Paris} en partenariat avec \textbf{EXAIL Robotics}.\\
%     \item \textbf{MOBITER}, de \textbf{SHERPA Engineering} en partenariat avec l'\textbf{INRAE} et L'\textbf{Institut Pascal}.
% \end{itemize}

\subsection{Les attendus du challenge}
\subsubsection{Organisation générale}
Le Challenge Mobilex est organisé par les acteurs suivant (\hyperref[fig4]{Figure 4}) :
...
L'organisation de chaque défi se déroule comme suit (\hyperref[fig5]{Figure 5}) :
...
Lors du troisième et dernier défi, les équipes se sont vue supprimés certaines fonctions imposées au cours des précédents défis, à savoir :\\
\begin{itemize}
\renewcommand{\labelitemi}{$\bullet$}
...
Et se sont vue rajoutés les nouvelles fonctions suivantes :\\
\begin{itemize}
\renewcommand{\labelitemi}{$\bullet$}
...
\indent La \hyperref[fig6]{Figure 6} est un tableau récapitulatif sur la franchissabilité des obstacles en fonction de leur hauteur et de la vitesse du robot :
...
\renewcommand{\labelitemi}{$\bullet$}
    ...
    \item LIDAR*\\
\end{itemize}
\indent Au cour du défi 2, ces pannes deuvaient pouvoir être simulées en coupant momentanément les capteurs souhaités par contre cette fonction n'est plus évaluer lors du défi 3. En cas de panne, les fonctions doivent toujours pouvoir être réalisées.
...
Pour répondre au challenge, la DGA a mis à disposition de chaque équipe un robot BARAKUDA de la société SHARK ROBOTICS (\hyperref[fig8]{Figure 8}).
...
\indent La communication entre le robot et la Brique MOBILEX* se fait via des trames UDP (protocole réseau utilisé pour des applications sensibles au temps) permettant d'envoyer des commandes de vitesse linéaires et angulaires et de récupérer des informations sur l'état du robot (vitesses et puissances des roues, inclinaisons, charge des batteries et température) par instruction de type GET.

\subsubsection{Matériel ajouté par le consortium}
Les équipes n'ont pas l'autorisation de modifier le robot en lui-même mais sont libres d'ajouter leur propre matériel tant qu'il respecte les dimensions à ne pas dépasser pour le challenge et la charge utile du robot. Nous avons donc choisi d'ajouter le matériel suivant (\hyperref[fig9]{Figure 9}):

\begin{figure}[H] % Utilisez [H] au lieu de [htb]
  \centering % Utilisez \centering au lieu de \makebox
  \includegraphics[width=0.8\linewidth]{Corps_rapport/Plateau_hardware.png}
  \caption{Plateau capteurs & hardware}
  \label{fig9}
\end{figure}
...
Ce Poste constitué d'un PC portable, d'antennes omni (5GHz et 868MHz), d'une manette de jeu, d'une batterie + onduleur permet au téléopérateur de contrôler le robot à distance et de recevoir les retours vidéo et d'informations sur l'état du robot. Le PCC se présente comme suit (\hyperref[fig10]{Figure 10}) :  \\\\


\begin{figure}[H] % Utilisez [H] au lieu de [htb]
  \centering % Utilisez \centering au lieu de \makebox
  \includegraphics[width=0.4\linewidth]{Corps_rapport/PCC.png}
...
On obtient alors sur le robot l'architecture hardware (\hyperref[fig11]{Figure 11}) :
\begin{center}
